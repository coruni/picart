...
Condition 是 JSON 格式，定义成就的完成条件。**注意：`condition.event` 仅作为文档标记，实际事件监听在 `achievement-event.service.ts` 中硬编码实现。**
...
| `event`  | string | **标记用**：说明关联的事件（实际路由在代码中硬编码）         |
...
> ⚠️ **重要说明**：`condition.event` 字段**仅用于文档标记**，实际的事件监听和成就更新逻辑在 [`achievement-event.service.ts`](../src/modules/achievement/achievement-event.service.ts) 中**硬编码实现**。
>
> 例如 `article.created` 事件会同时更新 `FIRST_ARTICLE`, `ARTICLE_10`, `ARTICLE_50`, `ARTICLE_100` 四个成就的进度。如需添加新的成就，需要在 `achievement-event.service.ts` 中添加对应的事件处理器调用。
