...
export class InviteController {
  ...
  async useInvite(
    @Request() req: Request & { user: User },
    @Body() useInviteDto: UseInviteDto,
  ) {
  ...
  async getMyInvites(
    ...
    return await this.inviteService.getMyInvites(
      req.user.id,
      pagination,
      keyword,
      status,
  ...
  @ApiResponse({ status: 200, description: "获取成功" })
  @ApiResponse({ status: 401, description: "未授权" })
  async getMyInviteEarnings(
    @Request() req: Request & { user: User },
    @Query() pagination: PaginationDto,
  ) {
    return await this.inviteService.getMyInviteEarnings(
      req.user.id,
      pagination,
    );
  }
