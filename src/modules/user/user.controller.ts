...
export class UserController {
  ...
  async registerUser(
    ...
    return this.userService.create(createUserDto, undefined, req);
  }
  ...
  create(
    ...
    return this.userService.create(createUserDto, req.user, req);
  }
  ...
  async withdrawWallet(
    ...
    return this.userService.withdrawWallet(
      req.user.id,
      data.amount,
      data.bankInfo,
    );
  ...
  async sendVerificationCode(@Body() data: SendMailDto) {
    return this.userService.sendVerificationCode(data.email, data.type);
  }
  ...
  @ApiResponse({ status: 400, description: "请求参数错误" })
  @ApiResponse({ status: 404, description: "用户不存在" })
  @ApiResponse({ status: 410, description: "验证码已过期" })
  async resetPassword(
  ...
  @UseGuards(AuthGuard("jwt"))
  @ApiOperation({ summary: "修改密码（需要原密码）" })
