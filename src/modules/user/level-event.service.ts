...
export class LevelEventService {
  ...
  async handleArticleCreated(payload: { userId: number; articleId: number }) {
    try {
      const canEarn = await this.checkDailyLimit(
        payload.userId,
        "PUBLISH_ARTICLE",
      );
      if (!canEarn) {
      ...
      console.log(
        `用户 ${payload.userId} 发布文章获得 ${this.EXP_CONFIG.PUBLISH_ARTICLE} 经验`,
      );
    } catch (error) {
  ...
  async handleArticleReceivedLike(payload: {
    authorId: number;
    articleId: number;
    likerId: number;
  }) {
    try {
      ...
      console.log(
        `用户 ${payload.authorId} 的文章被点赞获得 ${this.EXP_CONFIG.ARTICLE_RECEIVED_LIKE} 经验`,
      );
    } catch (error) {
  ...
  async handleArticleReceivedComment(payload: {
    ...
    try {
      ...
      console.log(
        `用户 ${payload.authorId} 的文章被评论获得 ${this.EXP_CONFIG.ARTICLE_RECEIVED_COMMENT} 经验`,
      );
    } catch (error) {
  ...
  async handleCommentCreated(payload: {
    ...
    try {
      ...
      console.log(
        `用户 ${payload.userId} 发表评论获得 ${this.EXP_CONFIG.PUBLISH_COMMENT} 经验`,
      );
    } catch (error) {
  ...
  async handleCommentReceivedLike(payload: {
    authorId: number;
    commentId: number;
    likerId: number;
  }) {
    try {
      ...
      console.log(
        `用户 ${payload.authorId} 的评论被点赞获得 ${this.EXP_CONFIG.COMMENT_RECEIVED_LIKE} 经验`,
      );
    } catch (error) {
  ...
  async handleDailyLogin(payload: { userId: number }) {
    try {
      ...
      console.log(
        `用户 ${payload.userId} 每日登录获得 ${this.EXP_CONFIG.DAILY_LOGIN} 经验`,
      );
    } catch (error) {
  ...
  async handleUserFollowed(payload: { userId: number; targetUserId: number }) {
    try {
      ...
      console.log(
        `用户 ${payload.userId} 关注用户获得 ${this.EXP_CONFIG.FOLLOW_USER} 经验`,
      );
    } catch (error) {
  ...
  async handleMembershipPurchased(payload: {
    userId: number;
    orderId: number;
  }) {
    try {
      ...
      console.log(
        `用户 ${payload.userId} 购买会员获得 ${this.EXP_CONFIG.PURCHASE_MEMBERSHIP} 经验`,
      );
    } catch (error) {
  ...
  async handleProfileCompleted(payload: { userId: number }) {
    try {
      ...
      console.log(
        `用户 ${payload.userId} 完善资料获得 ${this.EXP_CONFIG.COMPLETE_PROFILE} 经验`,
      );
    } catch (error) {
