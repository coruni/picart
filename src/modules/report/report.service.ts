...
export class ReportService {
  ...
  async create(createReportDto: CreateReportDto, reporterId: number) {
    const { type, reportedUserId, reportedArticleId, reportedCommentId } =
      createReportDto;
  ...
  async findAll(queryReportDto: QueryReportDto) {
    ...
    const queryBuilder = this.reportRepository
      ...
      .leftJoinAndSelect(
        ...
        "reportedUserDecorations",
      )
      .leftJoinAndSelect(
        "reportedUserDecorations.decoration",
        "reportedUserDecoration",
      )
    ...
    if (keyword) {
      queryBuilder.andWhere("report.reason LIKE :keyword", {
        keyword: `%${keyword}%`,
      });
    }
    ...
    const sanitizedData = data.map((report) => ({
      ...
      reporter: report.reporter
        ? sanitizeUser(processUserDecorations(report.reporter))
        : null,
      reportedUser: report.reportedUser
        ? sanitizeUser(processUserDecorations(report.reportedUser))
        : null,
      handler: report.handler
        ? sanitizeUser(processUserDecorations(report.handler))
        : null,
    }));
    ...
    return {
      ...
      reportedArticle: report.reportedArticle
        ? {
            ...
            author: report.reportedArticle.author
              ? sanitizeUser(
                  processUserDecorations(report.reportedArticle.author),
                )
              : null,
          }
        : null,
      reportedComment: report.reportedComment
        ? {
            ...
            author: report.reportedComment.author
              ? sanitizeUser(
                  processUserDecorations(report.reportedComment.author),
                )
              : null,
          }
        : null,
